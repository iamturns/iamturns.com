...
const TitleLong = ({ title }: TitleProps) => (
	<div className={classNames("sm:absolute sm:pin", "sm:flex sm:items-center sm:justify-center")}>
...
const TitleShort = ({ title }: TitleProps) => (
	<div className={classNames("absolute pin", "flex items-center justify-center")}>
...
const HeadingText = ({ title }: TitleProps) => (
	<h1 className={classNames("text-highlight", "text-2xl md:text-3xl")}>{title}</h1>
);
