...
const Content = () => (
	<div>
		{/* prettier-ignore */}
		<div>
			<ContentP>I assumed RxJS streams worked like Promises, and I was very wrong.</ContentP>
			...
			<ContentP>
				Where’s the meatballs?! It seems the error killed the stream.
			...
			<ContentP>
				...
				You expect to receive nicely structured JSON.
				Then one day, you don’t.
			</ContentP>

			<ContentP>Maybe you’re passing the “stuff” into a 3rd party library, and it throws an error.</ContentP>

			<ContentP>Errors will always happen. You can’t code defensively for every unpredictable possibility.</ContentP>
			...
			<ContentP>Meatballs! <Emoji>😍</Emoji></ContentP>

			<ContentP>But, what a nightmare to place this inside <em>every</em> operator:</ContentP>
			...
			<ContentP>The error was caught, but the stream completes before the meatballs had a chance.</ContentP>

			<ContentP>
				<Emoji>🤔</Emoji> Hmm. The catch operator is returning a bran new stream.
				Maybe the original stream is completing because it’s replaced?
			</ContentP>
			...
			<ContentP>
				This looks complex, but it’s behaving similar to the catch operator above.
				We are watching for an error notification, and then converting it into a next / success notification instead.
			</ContentP>

			<ContentP>It’s pretty low level, but this time we have no catch operator replacing the main stream. Should be fine.</ContentP>
			...
			<ContentP><Emoji>😖</Emoji> No meatballs.</ContentP>
			...
			<ContentP>
				RxJS is internally wrapping code in operators (like <ContentCode code="map" />, <ContentCode code="filter" />, etc) with a <ContentCode code="try / catch" />.
				If an error is caught, it will notify subscribers, and then <strong>unsubscribe</strong> the stream.
				This is <em>hard coded</em> — you have no choice!
			</ContentP>

			<ContentPullQuote>If an error is thrown within an operator, the stream will always complete.</ContentPullQuote>

			<ContentH2>Solution</ContentH2>

			<ContentP>Create a disposable stream. If an error occurs only the disposable stream dies, and the main stream lives on.</ContentP>
			...
			<ContentP>We have meatballs! <Emoji>😋</Emoji></ContentP>
			...
			<ContentUL>
				...
				<ContentLI>Keeping the main stream alive (so future responses are processed after an error)</ContentLI>
